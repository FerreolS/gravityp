 ...
 *-----------------------------------------------------------------------------
 *
 *	$Id: fits.i,v 1.29 2009/04/22 07:17:11 eric Exp eric $
 *	$Log: fits.i,v $
 *	Revision 1.29  2009/04/22 07:17:11  eric
 *	 - Heavy work on fits_read_bintable to support variable length
 *	   arrays.  Thanks to Jay Salmonson for starting this.
 *	 - Some bug fixes in fits_read_bintable for single precision
 *	   complexes and cases when the table can be read in one call
 *	   to _read.
   ...
   SEE ALSO: fits, fits_write_bintable, fits_pack_bintable. */
{
  ...
  if (nread >= 1) {
    ...
    /* Fix logical arrays: 'T' -> 1 (true), 'F' -> 0 (false), and any other
       character -> -1 (bad). */
    if (! raw_logical &&
        (n = numberof((subindex = where(field_type == _FITS_TFORM_LOGICAL)))) > 0) {
      subindex = index(subindex);
      for (k = 1; k <= n; ++k) {
        j = subindex(k);
    ...
    /* Fix string arrays. */
    if (! raw_string &&
        (n = numberof((subindex = where(field_type == _FITS_TFORM_STRING)))) > 0) {
      subindex = index(subindex);
      for (k = 1; k <= n; ++k) {
        j = subindex(k);
    ...
    if ((n = numberof((subindex = where(field_type == _FITS_TFORM_POINTER)))) > 0) {
      subindex = index(subindex);
      ...
      for (k = 1; k <= n; ++k) {
        /* Variable length array descriptor is two 32-bit integers: (number,offset). */
        j = subindex(k);
        ...
        if (ident == _FITS_TFORM_LOGICAL) {
          ...
          for (r = 1; r <= nrows; ++r) {
            if (parray(r)) {
              parray(r) = &_fits_bintable_fix_string(*parray(r), numberof(*parray(r)), trim);
            }
          }
